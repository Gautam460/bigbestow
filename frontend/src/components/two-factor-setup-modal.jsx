            ...
            {errors?.length ? (
                ...
                <>
                    ...
                    <div className="flex w-full space-x-5">
                        <Button className="w-full font-bold" onClick={onNextStep}>
                            {buttonText}
                        </Button>
                    </div>
                            ...
                            {!manualSetupKey ? (
                                ...
                                <>
                                    <input
                                        ...
                                        className="h-full w-full bg-background p-3 text-foreground font-mono text-sm outline-none"
                                    />
                                    <button
...
function TwoFactorVerificationStep({ onClose, onBack }) {
    ...
    const handleConfirm = async (e) => {
        ...
        setError('');
        try {
            await api.post('/api/user/confirmed-two-factor-authentication', { code });
            toast.success('Two-factor authentication confirmed and enabled.');
            onClose();
        } catch (err) {
            setError(err?.response?.data?.errors?.code?.[0] || 'Invalid code provided.');
        } finally {
            setProcessing(false);
        }
    };
