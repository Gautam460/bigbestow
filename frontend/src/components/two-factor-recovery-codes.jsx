...
export default function TwoFactorRecoveryCodes({
    ...
    const toggleCodesVisibility = useCallback(async () => {
        if (!codesAreVisible && !recoveryCodesList.length && fetchRecoveryCodes) {
    ...
    const handleRegenerate = async () => {
        setProcessing(true);
        try {
            await api.post('/api/user/two-factor-recovery-codes');
            toast.success('Recovery codes regenerated successfully.');
            if (fetchRecoveryCodes) await fetchRecoveryCodes();
        } catch (err) {
            ...
            <CardContent>
                <div className="flex flex-col gap-3 select-none sm:flex-row sm:items-center sm:justify-between">
                    ...
                    {canRegenerateCodes && (
                        ...
                        >
                            <RefreshCw className={processing ? 'animate-spin mr-2' : 'mr-2'} /> Regenerate codes
                        </Button>
                    )}
                </div>
                <div
