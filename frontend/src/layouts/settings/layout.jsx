...
export default function SettingsLayout({ children }) {
    ...
    return (
        <EcommerceLayout>
            <div className="bg-gray-50/50 dark:bg-slate-950 min-h-screen py-10 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-300">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-purple-900 rounded-3xl p-8 mb-8 text-white shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6">
                        ...
                        <div className="flex items-center gap-3">
                            <button onClick={() => logout && logout()} className="px-5 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                        ...
                        <aside className="lg:col-span-1">
                            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200/80 dark:border-slate-800/80 p-4 shadow-sm space-y-1 transition-colors duration-300">
                                <p className="text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest px-3 py-2">Account Navigation</p>
                                {navItems.map((item) => {
                                        ...
                                        <Link
                                            ...
                                            className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 ${
                                                active
                                                    ...
                                                    : 'text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800/80 hover:text-indigo-600 dark:hover:text-indigo-400'
                                            }`}
                                        >
                                            <div className="flex items-center gap-3">
                                                <item.icon className={`w-5 h-5 ${active ? 'text-yellow-300' : 'text-gray-400 dark:text-slate-500'}`} />
                        ...
                        <div className="lg:col-span-3">
                            <div className="bg-white dark:bg-slate-900 dark:text-slate-100 rounded-2xl border border-gray-200/80 dark:border-slate-800/80 p-6 sm:p-8 shadow-sm transition-colors duration-300">
                                {children}
                            </div>
                        </div>
                    </div>
