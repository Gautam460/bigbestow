...
import { Package, Clock, CheckCircle, Truck, XCircle, ArrowLeft, MapPin, CreditCard, Tag, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function OrderShowPage() {
    ...
    const fetchOrder = () => {
        if (!id) return;
        setLoading(true);
        api.get(`/api/user/orders/${id}`).then(res => {
            if (res && res.order) setOrder(res.order);
            else if (res && res.id) setOrder(res);
            else if (res && res.data) setOrder(res.data);
            else setOrder(null);
        }).catch(() => {
            setOrder(null);
        }).finally(() => {
    ...
    const handleCancelOrder = async () => {
        ...
        if (confirm(`Are you sure you want to cancel Order #ORD-${order.id}?`)) {
            try {
                await api.post(`/api/user/orders/${order.id}/cancel`);
    ...
    const getStatusBadge = (status) => {
        switch (status?.toLowerCase()) {
            ...
            default:
                return <span className="px-3.5 py-1.5 bg-gray-100 text-gray-800 rounded-full text-xs font-black uppercase tracking-wider">{status || 'Processing'}</span>;
        }
    };
        ...
        return (
            <SettingsLayout>
                <div className="text-center py-16">
                    ...
                    <p className="text-sm font-bold text-gray-500">Loading order details...</p>
                </div>
            </SettingsLayout>
        );
    }
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-100">
                        <div>
                            ...
                            <p className="text-sm font-semibold text-gray-500 mt-1">
                                Placed on {order.created_at ? new Date(order.created_at).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'Recent'}
                            </p>
                        </div>
                        <div>
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            ...
                            <div className="divide-y divide-gray-100 border border-gray-200 rounded-2xl overflow-hidden">
                                {order.items?.map((item, idx) => (
                                    <div key={item.id || idx} className="p-4 bg-white flex items-center justify-between gap-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 font-semibold">Unit Price: ₹{Number(item.price || 0).toFixed(2)}</p>
                                            </div>
                                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gray-50 rounded-2xl p-6 border border-gray-200/60 space-y-3">
                                ...
                                <div className="space-y-2 text-xs">
                                    ...
                                    <div className="flex justify-between text-gray-600">
                                        <span>Subtotal</span>
                                        <span className="font-bold text-gray-900">₹{(Number(order.total_amount || 0) + Number(order.discount_amount || 0)).toFixed(2)}</span>
                                    </div>
                                    ...
                                    <div className="flex justify-between pt-3 border-t border-gray-200 text-sm">
                                        ...
                                        <span className="font-black text-indigo-600 text-base">₹{Number(order.total_amount || 0).toFixed(2)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
