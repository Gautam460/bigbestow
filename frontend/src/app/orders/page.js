...
import { Package, Clock, CheckCircle, Truck, XCircle, Eye, ArrowRight, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function OrdersPage() {
    ...
    const getStatusBadge = (status) => {
        switch (status?.toLowerCase()) {
            ...
            default:
                return <span className="px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-xs font-black uppercase tracking-wider">{status || 'Processing'}</span>;
        }
    };
                                        ...
                                        <div className="bg-gray-50/80 px-6 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
                                            <div className="flex flex-wrap items-center gap-6">
                                                ...
                                                <div>
                                                    ...
                                                    <span className="text-base font-black text-indigo-600">₹{Number(order.total_amount || 0).toFixed(2)}</span>
                                                </div>
                                                <div>
                                            ...
                                            <div className="bg-gray-50/50 rounded-xl p-4 mb-6 border border-gray-100">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                                    {order.items?.slice(0, 3).map((item, idx) => (
                                                        <div key={item.id || idx} className="flex items-center gap-3 bg-white p-2.5 rounded-lg border border-gray-200/60 shadow-xs">
                                                            ...
                                                            <div className="min-w-0 flex-1">
                                                                ...
                                                                <span className="text-[11px] text-gray-500 font-semibold">₹{Number(item.price || 0).toFixed(2)} each</span>
                                                            </div>
                                                        </div>
                                                    ))}
