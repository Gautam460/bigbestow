...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        api.get('/api/products').then(res => {
            if (Array.isArray(res)) setRecommendedProducts(res.slice(0, 4));
            else if (res && Array.isArray(res.data)) setRecommendedProducts(res.data.slice(0, 4));
            else if (res && Array.isArray(res.products)) setRecommendedProducts(res.products.slice(0, 4));
        }).catch(() => {});
    }, []);
    ...
    const displayProducts = recommendedProducts.length > 0 ? recommendedProducts : fallbackProducts;
    ...
    return (
        <SettingsLayout>
            <div className="space-y-8">
                ...
                {/* Header Section */}
                <div>
                    <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Account Overview</h2>
                    <p className="text-gray-500 dark:text-slate-400 text-sm">Welcome back! Manage your profile, orders, and personalized shopping recommendations.</p>
                </div>

                {/* Stats Overview */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-600/80 shadow-sm flex items-center gap-4">
                        <div className="p-3.5 bg-indigo-50 rounded-xl text-indigo-600">
                            <ShoppingBag className="w-6 h-6" />
                        </div>
                        <div>
                            <div className="text-2xl font-black text-gray-900 dark:text-white">{displayOrders.length}</div>
                            <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">Total Orders</div>
                        </div>
                    </div>
                    <Link href="/wishlist" className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-600/80 shadow-sm hover:shadow-md transition-all flex items-center gap-4">
                        <div className="p-3.5 bg-rose-50 rounded-xl text-rose-600">
                            <Heart className="w-6 h-6" />
                        </div>
                        <div>
                            <div className="text-2xl font-black text-gray-900 dark:text-white">{wishlistIds.length}</div>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-600/80 shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <div className="text-2xl font-black text-gray-900 dark:text-white">02</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {/* Recent Orders */}
                    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-600/80 shadow-sm p-6 flex flex-col justify-between">
                        <div>
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-lg font-black text-gray-900 dark:text-white tracking-tight">Recent Orders</h3>
                                <Link href="/orders" className="text-xs font-bold text-indigo-600 hover:underline">View All Orders</Link>
                            </div>
                            
                            <div className="space-y-4">
                                {displayOrders.map((order, i) => (
                                    <Link key={i} href={order.id && !String(order.id).startsWith('#') ? `/orders/${order.id}` : '/orders'} className="flex items-center justify-between p-4 rounded-xl border border-gray-100 dark:border-slate-700 hover:border-indigo-200 hover:shadow-md transition-all cursor-pointer group bg-white dark:bg-slate-800">
                                        <div className="flex items-center gap-4">
                                            <div className="p-2.5 bg-gray-50 dark:bg-slate-900 rounded-xl text-gray-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
                                                <Package className="w-5 h-5" />
                                            </div>
                                            <div>
                                                <div className="font-bold text-gray-900 dark:text-white text-sm group-hover:text-indigo-600 transition-colors">
                                                    {order.id ? (String(order.id).startsWith('#') ? order.id : `#ORD-${order.id}`) : (order.order_number || `#ORD-${100 + i}`)}
                                                </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-600/80 shadow-sm p-6">
                            <h3 className="text-lg font-black text-gray-900 dark:text-white tracking-tight mb-6">Security & Profile Tracking</h3>
                            <div className="space-y-4">
                                <div className="flex items-center justify-between p-4 bg-indigo-50/50 rounded-xl border border-indigo-100">
                                    <div className="flex items-center gap-3">
                                        <Clock className="w-5 h-5 text-indigo-600" />
                                        <span className="text-sm font-bold text-gray-700 dark:text-slate-300">Account Security</span>
                                    </div>
                                ...
                                <div className="flex items-center justify-between p-4 bg-emerald-50/50 rounded-xl border border-emerald-100">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-bold text-gray-700 dark:text-slate-300">Customer Profile</span>
                                    </div>
                ...
                <div className="pt-6 border-t border-gray-200 dark:border-slate-600">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">Recommended For You</h3>
                            <p className="text-xs font-bold text-gray-500 dark:text-slate-400 mt-0.5">Inspired by your shopping trends & browsing history</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {displayProducts.map((product) => (
                            <div key={product.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-600/80 overflow-hidden shadow-sm hover:shadow-lg transition-all group flex flex-col justify-between">
                                <div>
                                    ...
                                    <div className="p-4">
                                        <div className="flex items-center gap-1 text-yellow-400 mb-1.5">
                                            ...
                                            <span className="text-xs font-bold text-gray-500 dark:text-slate-400 ml-1">(4.9)</span>
                                        </div>
                                        <h4 className="font-black text-gray-900 dark:text-white text-sm line-clamp-2 group-hover:text-indigo-600 transition-colors">
                                            {product.name}
                                        </h4>
                                    </div>
                                </div>
                                <div className="p-4 pt-0 flex items-center justify-between border-t border-gray-100 dark:border-slate-700 mt-4">
                                    <span className="text-lg font-black text-gray-900 dark:text-white">₹{product.price || '9999.00'}</span>
                                    <Link
                                        href={`/products`}
