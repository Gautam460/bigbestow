    ...
    return (
        <EcommerceLayout>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-[60vh]">
                <h1 className="text-3xl font-black text-gray-900 dark:text-white mb-8">My Wishlist <span className="text-gray-400 font-normal text-xl">({wishlistItems.length} items)</span></h1>
                
                {wishlistItems.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {wishlistItems.map((item) => (
                            <div key={item.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden group hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
                                <div>
                                    <div className="relative h-60 overflow-hidden bg-gray-100 dark:bg-slate-800">
                                        ...
                                        <button 
                                            ...
                                            className="absolute top-3 right-3 bg-white dark:bg-slate-800 p-2.5 rounded-full shadow-md text-rose-500 hover:scale-110 transition-all z-10"
                                        >
                                    ...
                                    <div className="p-4">
                                        ...
                                        <Link href={`/products/${item.slug || item.id}`}>
                                            <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-2 truncate text-lg hover:text-indigo-600 transition-colors">{item.name}</h3>
                                        </Link>
                                        <div className="text-xl font-black text-gray-900 dark:text-white mb-4">₹{item.price}</div>
                                    </div>
                                </div>
                    ...
                    <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm">
                        ...
                        <p className="text-gray-500 dark:text-slate-400 mb-6">Save items you love here to buy them later.</p>
