        ...
        <EcommerceLayout>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100 dark:border-slate-700">
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 dark:text-white">Your Shopping Cart</h1>
                        <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">{cartItems.length} {cartItems.length === 1 ? 'item' : 'items'} in your bag</p>
                    </div>
                    {cartItems.length > 0 && (
                        <button 
                            onClick={handleClear}
                ...
                {cartItems.length === 0 ? (
                    <div className="bg-white dark:bg-slate-800 rounded-3xl p-16 text-center shadow-sm border border-gray-100 dark:border-slate-700 max-w-2xl mx-auto">
                        ...
                        <p className="text-gray-500 dark:text-slate-400 mb-8">Looks like you haven&apos;t added any professional cricket gear to your cart yet.</p>
                        <Link 
                            href="/products" 
                    ...
                    <div className="flex flex-col lg:flex-row gap-10">
                        ...
                        <div className="w-full lg:w-2/3">
                            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 sm:p-8 space-y-6">
                                {cartItems.map((item) => (
                                    <div key={item.id} className="flex flex-col sm:flex-row gap-6 pb-6 border-b border-gray-100 dark:border-slate-700 last:border-0 last:pb-0">
                                        <Link href={`/products/${item.id}`} className="w-full sm:w-32 h-32 rounded-2xl bg-gray-100 dark:bg-slate-900 overflow-hidden flex-shrink-0">
                                            <img 
                                            ...
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <Link href={`/products/${item.id}`}>
                                                        <h3 className="font-bold text-gray-900 dark:text-white text-lg hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">{item.name}</h3>
                                                    </Link>
                                                    <p className="text-gray-500 dark:text-slate-400 text-xs font-semibold mt-1 bg-gray-100 dark:bg-slate-700 inline-block px-2.5 py-1 rounded">
                                                        Handle: {item.attributes?.size || 'Standard SH'}
                                                    </p>
                                                </div>
                                                <div className="text-xl font-black text-gray-900 dark:text-white">₹{(item.price * item.quantity).toFixed(2)}</div>
                                            </div>
                                            <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-50 sm:border-0 sm:pt-0">
                                                <div className="flex items-center border-2 border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-800">
                                                    <button 
                                                        ...
                                                        className="px-3.5 py-1 text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 font-black text-base rounded-l-lg transition-colors"
                                                    >-</button>
                                                    <span className="px-4 font-black text-gray-900 dark:text-white text-sm">{item.quantity}</span>
                                                    <button 
                                                        onClick={() => handleQuantityChange(item.id, item.quantity + 1)}
                                                        className="px-3.5 py-1 text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 font-black text-base rounded-r-lg transition-colors"
                                                    >+</button>
                                                </div>
                                                <button 
                                ...
                                {appliedCoupon ? (
                                    ...
                                    <form onSubmit={handleApplyCoupon} className="flex gap-2">
                                        <input 
                                            ...
                                            className="flex-1 px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-600 rounded-xl text-sm font-bold text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-600 uppercase tracking-wider"
                                        />
                                        <button 
                                            type="submit"
                                            disabled={isApplying || !couponCode.trim()}
                                            className="px-5 py-3 bg-gray-900 dark:bg-slate-700 hover:bg-indigo-600 dark:hover:bg-indigo-600 text-white font-bold text-sm rounded-xl transition-all disabled:opacity-50 flex items-center justify-center"
                                        >
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-slate-700 shadow-sm sticky top-32">
                                ...
                                <div className="border-t border-gray-100 dark:border-slate-700 pt-4 mb-8">
                                    <div className="flex justify-between items-baseline">
                                        <span className="font-black text-gray-900 dark:text-white text-lg">Total Amount</span>
                                        <span className="font-black text-3xl text-indigo-600 dark:text-indigo-400">₹{total.toFixed(2)}</span>
                                    </div>
                                    <p className="text-[11px] text-gray-400 dark:text-slate-500 mt-1">Inclusive of all taxes and shipping fees</p>
                                </div>
                                ...
                                <div className="flex items-center justify-center gap-4 text-gray-400 dark:text-slate-500 mt-6 pt-6 border-t border-gray-100 dark:border-slate-700">
                                    <ShieldCheck className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                                    <CreditCard className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                                    <span className="text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-slate-400">100% Secure Checkout</span>
                                </div>
                            </div>
                        </div>
                    </div>
